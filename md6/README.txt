...
and adds or replaces MD6 hashes in each file record before outputing the
manifest file to standard output.

In update mode, the program takes an Opal manifest file stream from standard
input and checks modification times before replacing MD6 hashes in each file
record or adds a hash to a record with no hash before outputing the manifest
file to standard output.

In check mode, the program reads a manifest file, looks for MD6 hashes to
...
": NO HASH" if there is no associated MD6 hash, or ": OLD HASH" if the
associated MD6 hash is for a file that has a different modification time and
...
oumnf-md6.txt describes the extensions that are made to the manifest format to
support MD6 hashes
